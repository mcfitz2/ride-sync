	...
	async.each(days, function(day, callback) {
		...
		db.Ride.find({where:{day:date, UserId:user.id}}).done(function(err, ride) {
			...
			strava.uploads.upload({
			    ...
			    var activity_id = res.activity_id || /duplicate of activity ([0-9]+)/.exec(res.error)[1];
			    db.Ride.create({day:date, activity_id:activity_id, gpx:gpx}).done(function(err, ride) {
				callback(err);
			    });
			});
		    } else {
