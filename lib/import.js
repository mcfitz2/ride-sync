...
function getSegments(moves, user, callback) {
	...
	var localZone = body.profile.currentTimeZone.offset/60;
	var first =  moment(body.profile.firstDate, "YYYYMMDD");
	var dates = mkDates(moment(user.last_fetch || first));
	var today = moment().zone(-4).set("hours", 0).set("minutes", 0).set("milliseconds", 0).set("seconds", 0);
	...
	async.concat(dates, function(date, callback) {
		...
		async.concat(days, function(day, callback) {
		    var fixedDate = moment(day.date, "YYYYMMDD");
...
module.exports = function(callback) {
	...
	async.each(days, function(day, callback) {
		...
		db.Ride.find(date, {where:{UserId:user.id}}).done(function(err, ride) {
		    if (!ride) {
			strava.uploads.upload({
			    ...
			    db.Ride.create({day:date, activity_id:res.activity_id, gpx:gpx}).done(function(err, ride) {
				callback(err);
			    });
			});
		    } else {
